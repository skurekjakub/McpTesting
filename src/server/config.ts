...
const DEFAULT_SUMMARIZATION_MODEL_FALLBACK = 'gemini-2.0-flash';
// For cost optimization, use a smaller model for aggressive summarization
const DEFAULT_COST_EFFICIENT_SUMMARIZATION_MODEL_FALLBACK = 'gemini-1.0-pro';
...
const ConfigSchema = z.object({
  ...
  // New: Smaller model for cost-efficient summarization
  COST_EFFICIENT_SUMMARIZATION_MODEL: z.string().default(DEFAULT_COST_EFFICIENT_SUMMARIZATION_MODEL_FALLBACK),
  SUMMARIZATION_THRESHOLD_TOKENS: z.number().int().positive().default(DEFAULT_SUMMARIZATION_THRESHOLD_TOKENS),
  // New: Cost optimization configuration
  COST_OPTIMIZATION_ENABLED: z.boolean().default(true),
  RECENT_MESSAGES_TO_PRESERVE: z.number().int().positive().default(6),
  DEEP_HISTORY_THRESHOLD: z.number().int().positive().default(8000),
...
const combinedConfig = {
  ...
  // New: Add cost-efficient summarization model
  COST_EFFICIENT_SUMMARIZATION_MODEL: configData.cost_efficient_summarization_model,
  SUMMARIZATION_THRESHOLD_TOKENS: configData.summarization_threshold_tokens, // Let Zod handle default
  // New: Cost optimization configuration
  COST_OPTIMIZATION_ENABLED: configData.cost_optimization_enabled,
  RECENT_MESSAGES_TO_PRESERVE: configData.recent_messages_to_preserve,
  DEEP_HISTORY_THRESHOLD: configData.deep_history_threshold,
...
try {
  ...
  logger.info(`  Cost-Efficient Summarization Model: ${validatedConfig.COST_EFFICIENT_SUMMARIZATION_MODEL}`);
  logger.info(`  Summarization Threshold: ${validatedConfig.SUMMARIZATION_THRESHOLD_TOKENS} tokens`);
  logger.info(`  Cost Optimization Enabled: ${validatedConfig.COST_OPTIMIZATION_ENABLED}`);
  logger.info(`  Recent Messages to Preserve: ${validatedConfig.RECENT_MESSAGES_TO_PRESERVE}`);
  logger.info(`  Deep History Threshold: ${validatedConfig.DEEP_HISTORY_THRESHOLD} tokens`);
