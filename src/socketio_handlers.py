...
from .logic import chat_processor, utils
...
def _deserialize_history(session_data: List[dict], sid: str) -> Optional['DeserializedHistory']:
    ...
    """
    try:
        history: 'DeserializedHistory' = [genai_types.Content.model_validate(item) for item in session_data]
        return history
    except Exception as e:
        error_trace = traceback.format_exc()
        utils.add_debug_log(f"Error deserializing history from session for {sid}: {e}\n{error_trace}")
        emit('new_message', {'type': 'error', 'text': f"Error loading chat history: {e}. History reset."}, room=sid)
...
def _serialize_history(history: 'DeserializedHistory', sid: str) -> Tuple['SerializedHistory', bool]:
    ...
    for i, item in enumerate(history):
        try:
            ...
            error_trace = traceback.format_exc()
            utils.add_debug_log(f"Error serializing history item #{i} for {sid}: {e_ser}\n{error_trace}")

    if serialization_error_occurred:
        utils.add_debug_log(f"Serialization error occurred for {sid}, message not emitted directly from here.")
...
def process_user_message(socketio_instance: 'SocketIO', background_loop: 'AbstractEventLoop', data: dict) -> None:
    ...
    gemini_history_internal: Optional['DeserializedHistory'] = _deserialize_history(gemini_history_internal_raw, client_sid)
    ...
    def emit_internal_step(message: str) -> None:
        ...
        try:
            socketio_instance.emit('new_message', {'type': 'internal', 'text': message}, room=client_sid)
        except Exception as e:
    ...
    async def process_chat_task() -> None:
        ...
        try:
            ...
            socketio_instance.emit('status_update', {'message': 'Processing...'}, room=client_sid)
            ...
            socketio_instance.emit('new_message', {'type': response_type, 'text': final_response_text}, room=client_sid)
            current_display_history.append({'type': response_type, 'text': final_response_text})

            serialized_history: 'SerializedHistory'
            serialization_error: bool
            serialized_history, serialization_error = _serialize_history(updated_gemini_history, client_sid)
            session['gemini_history_internal'] = serialized_history
            if serialization_error:
                socketio_instance.emit('new_message', {'type': 'error', 'text': "Error saving full chat history state."}, room=client_sid)
            ...
            socketio_instance.emit('new_message', {'type': 'error', 'text': f"Critical server error during processing: {e}"}, room=client_sid)
