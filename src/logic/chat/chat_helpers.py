...
async def get_available_tools(
            ...
            try:
                ...
                if internal_step_callback:
                    internal_step_callback(
                        f"Found {len(result)} tools from '{server_id}'."
                    )
            except Exception as format_e:
                error_msg = (
                    f"Error formatting tools from server '{server_id}': {format_e}"
                )
                utils.add_debug_log(error_msg)
                if internal_step_callback:
    ...
    utils.add_debug_log(
        f"Total formatted tools collected for Gemini: {len(all_gemini_tools)}"
    )
    ...
    try:
        ...
        try:
            token_count_response = (
                gemini_client_instance.models.count_tokens(  # Use passed client
                    model=config.GENERATION_GEMINI_MODEL, contents=history
                )
            )
            ...
            utils.add_debug_log(
                f"Warning: Failed to count tokens before API call: {count_e}"
            )
        # --- End Token Count ---
        ...
        utils.add_debug_log(
            f"Gemini response received. Candidates: {len(response.candidates) if response and response.candidates else 'N/A'}"
        )
        ...
        if not response or not response.candidates:
            ...
            error_msg = (
                f"Warning: Gemini response had no candidates. Feedback: {feedback}"
            )
            utils.add_debug_log(error_msg)
            if internal_step_callback:
                internal_step_callback(error_msg)
...
async def process_candidate(
    ...
    """
    utils.add_debug_log(
        f"Processing candidate 0. Finish Reason: {candidate.finish_reason}. Safety: {candidate.safety_ratings}"
    )
    ...
    if function_call_requested:
        utils.add_debug_log(
            f"Gemini requested function call #{function_call_iter}: {function_call_requested.name}"
        )
        ...
        function_response_part = await tool_handler.handle_function_call(
            ...
            mcp_servers_dict,
            internal_steps_list,  # Use passed mcp_servers
        )
        ...
        utils.add_debug_log(
            f"Added function response for {function_call_requested.name} to history."
        )
        return None, True

    else:
        utils.add_debug_log(
            "No function call requested by Gemini. Assuming final text response."
        )
