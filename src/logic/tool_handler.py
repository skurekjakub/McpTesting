...
async def discover_and_format_tools(
    ...
    for server_id, result in zip(mcp_servers.keys(), tool_results):
        if isinstance(result, Exception):
            utils.add_debug_log(f"Error listing tools from server '{server_id}': {result}")
...
def find_server_for_tool(
    ...
    utils.add_debug_log(f"Warning: Tool '{tool_name}' not found in any known server's cached tools.")
    return None
...
async def handle_function_call(
    ...
    if not target_server:
        ...
        utils.add_debug_log(f"Calling tool '{tool_name}' on server '{target_server.server_id}' with args: {tool_args}")
        tool_result = await target_server.call_tool(tool_name, arguments=tool_args)
        utils.add_debug_log(f"MCP tool '{tool_name}' raw result (preview): {str(tool_result)[:config.LOG_PREVIEW_LEN]}...")
        ...
        if tool_result is None:
            ...
            tool_output_content_str = f"Error extracting output from tool '{tool_name}'." # Default
            try:
                ...
                if hasattr(tool_result, 'content') and isinstance(tool_result.content, list) and len(tool_result.content) > 0:
                    ...
                    if hasattr(part, 'text') and part.text is not None:
                        ...
                        output_preview = (str(tool_output_content_str)[:config.LOG_PREVIEW_LEN] + '...' if len(str(tool_output_content_str)) > config.LOG_PREVIEW_LEN else str(tool_output_content_str)).replace('\n', ' ')
                        utils.add_debug_log(f"Tool '{tool_name}' output preview: {output_preview}")
                    else: # Part exists but no .text
                        internal_steps.append(f"Warning: Tool '{tool_name}' result part lacks 'text'. Using raw part representation.")
                        utils.add_debug_log(f"Tool '{tool_name}' result structure issue (no text): {part}")
                        tool_output_content_str = str(part)
                else: # No .content or it's empty/not a list
                    internal_steps.append(f"Warning: Tool '{tool_name}' result has unexpected structure or no content. Using raw result.")
                    utils.add_debug_log(f"Tool '{tool_name}' result structure issue (no content): {tool_result}")
                ...
                function_response_part = genai_types.FunctionResponse(
                 ...
                 error_msg = f"Error processing result from tool '{tool_name}': {extraction_err}"
    ...
    if not function_response_part:
         utils.add_debug_log(f"CRITICAL: function_response_part not created for tool {tool_name} after processing.")
         internal_steps.append(f"Internal error preparing response for tool {tool_name}")
         function_response_part = genai_types.FunctionResponse(
             name=tool_name,
             response={"error": f"Internal error processing result for tool '{tool_name}'."}
         )
