...
def validate_config():
    ...
    if not GEMINI_API_KEY:
        print("ERROR: Gemini API Key is missing. Set GEMINI_API_KEY environment variable or 'gemini_api_key' in config.json.")
        valid = False
    ...
    if not isinstance(FILESYSTEM_TARGET_DIRECTORIES, list):
         print(f"ERROR: 'filesystem_target_directories' in {CONFIG_FILENAME} must be a list of strings.")
         valid = False
    elif not FILESYSTEM_TARGET_DIRECTORIES: # Check if list is empty
        print(f"WARNING: 'filesystem_target_directories' is empty in {CONFIG_FILENAME}. Filesystem server tools will not be available.")
        # Don't mark as invalid, just warn, as memory server might still be used
        # valid = False
    else:
        print(f"  Filesystem Target Directories ({len(FILESYSTEM_TARGET_DIRECTORIES)}):")
        ...
        for i, dir_path in enumerate(FILESYSTEM_TARGET_DIRECTORIES):
            if not isinstance(dir_path, str):
                ...
                print(f"    ERROR: Directory '{dir_path}' (item #{i+1}) not found or is not a directory.")
                all_fs_paths_valid = False
            else:
                 print(f"    - '{dir_path}' [OK]")
    ...
    if not isinstance(ENABLE_MEMORY_SERVER, bool):
        print(f"ERROR: 'enable_memory_server' in {CONFIG_FILENAME} must be a boolean (true or false).")
        valid = False
    else:
    ...
    if not FILESYSTEM_TARGET_DIRECTORIES and not ENABLE_MEMORY_SERVER:
        print("ERROR: No MCP servers are configured. 'filesystem_target_directories' is empty AND 'enable_memory_server' is false.")
        valid = False
...
if not config_valid:
    print("--- Configuration errors detected. Application might not function correctly. ---")
