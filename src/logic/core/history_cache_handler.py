...
if TYPE_CHECKING:
    ...
    CachedData = Dict[
        str, Any
    ]  # {"gemini_history_internal": ..., "chat_history_display": ...}
    ...
    try:
        ...
        if cached_data and isinstance(cached_data, dict):
            ...
            if not isinstance(cached_data.get("chat_history_display"), list):
                utils.add_debug_log(
                    f"[{sid}] Warning: chat_history_display from cache was not a list, resetting."
                )
                cached_data["chat_history_display"] = []
            return cached_data
        else:
            ...
            utils.add_debug_log(
                f"[{sid}] No valid cache data found, returning default structure."
            )
            return {"gemini_history_internal": [], "chat_history_display": []}
    except Exception as e:
        utils.add_debug_log(
            f"[{sid}] Error getting data from cache: {e}\n{traceback.format_exc()}"
        )
        return {"gemini_history_internal": [], "chat_history_display": []}
...
def save_cached_data(
    ...
    try:
        ...
        utils.add_debug_log(
            f"[{sid}] Failed to save data to cache: {e}\n{traceback.format_exc()}"
        )
        return False
...
def reset_cache_for_sid(sid: str, error_message: Optional[str] = None) -> None:
    ...
    try:
        ...
        utils.add_debug_log(
            f"[{sid}] Failed to reset cache entry: {e}\n{traceback.format_exc()}"
        )
...
def delete_cache_for_sid(sid: str) -> None:
    ...
    try:
        ...
        utils.add_debug_log(
            f"[{sid}] Failed to delete cache entry: {e}\n{traceback.format_exc()}"
        )
