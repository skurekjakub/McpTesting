...
async def count_history_tokens(history: List[genai_types.Content],
    ...
    if not model_for_counting:
        utils.add_debug_log("Warning: Token counting called without a valid model instance.")
        return 0
    ...
    try:
        ...
        utils.add_debug_log(f"Token count for history ({len(history)} messages): {response.total_tokens}")
        return response.total_tokens
    except Exception as e:
        utils.add_debug_log(f"Error counting tokens: {e}")
        # Fallback: estimate based on characters (very rough)
        char_count = sum(part.text for content in history for part in content.parts if hasattr(part, 'text') and part.text)
        estimated_tokens = char_count // 4 # Rough estimate
        utils.add_debug_log(f"Falling back to estimated token count: {estimated_tokens}")
        return estimated_tokens

# --- Helper Function for Summarization ---
async def summarize_history(history: List[genai_types.Content],
                            target_tokens: int,
    ...
    if not summarization_model:
        utils.add_debug_log("Error: Summarization attempted without a valid model instance.")
        return None

    utils.add_debug_log(f"Attempting to summarize history ({len(history)} messages) using model: {summarization_model.model_name}")

    # Construct the summarization prompt
    history_text = "\n---\n".join(
        f"{content.role}: {part.text}"
        for content in history
        for part in content.parts if hasattr(part, 'text') and part.text # Ensure part.text exists
    )

    if not history_text.strip():
        utils.add_debug_log("Warning: History text for summarization is empty. Skipping summarization.")
...
Concise Summary:"""
    ...
    try:
        ...
        if response and response.text:
            ...
            utils.add_debug_log(f"Summarization successful. Summary length: {len(summary_text)} chars.")

            # Create the summarized history representation
            summarized_history = [
                genai_types.Content(
                    parts=[genai_types.Part(text=f"The conversation history up to this point has been summarized to save space. Key points:\n{summary_text}")],
            ...
            try:
                 summary_token_count = await count_history_tokens(summarized_history, summarization_model)
                 utils.add_debug_log(f"Actual summary token count: {summary_token_count} (target was ~{target_tokens})")
            except Exception as count_e:
                 utils.add_debug_log(f"Could not count tokens for the generated summary: {count_e}")


            return summarized_history
        else:
            feedback = response.prompt_feedback if response else "N/A"
            utils.add_debug_log(f"Summarization failed: No text in response. Feedback: {feedback}")
            # Consider safety ratings if available in response.candidates[0].safety_ratings
            if response and response.candidates and response.candidates[0].finish_reason == genai_types.FinishReason.SAFETY:
                 utils.add_debug_log(f"Summarization blocked by safety filter. Ratings: {response.candidates[0].safety_ratings}")
            return None # Indicate failure

    except Exception as e:
        error_trace = traceback.format_exc()
        utils.add_debug_log(f"Error during summarization API call with {summarization_model.model_name}: {e}\n{error_trace}")
        return None # Indicate failure
...
async def manage_history_tokens(
    ...
    try:
        current_token_count = await count_history_tokens(gemini_history, main_model_instance)
        utils.add_debug_log(f"History token count check: {current_token_count}/{MAX_HISTORY_TOKENS}")

        if current_token_count > MAX_HISTORY_TOKENS:
            utils.add_debug_log(f"Token count ({current_token_count}) exceeds limit ({MAX_HISTORY_TOKENS}). Triggering summarization.")

            # Get the summarization model instance
            summary_model_to_use = summarization_model_instance
            if not summary_model_to_use:
                 try:
                      utils.add_debug_log(f"Initializing dedicated summarization model: {config.SUMMARIZATION_MODEL_NAME}")
                      # Assumes genai is configured (API key) in the main script
                      summary_model_to_use = genai.GenerativeModel(config.SUMMARIZATION_MODEL_NAME)
                 except Exception as model_init_e:
                      utils.add_debug_log(f"Failed to initialize summarization model {config.SUMMARIZATION_MODEL_NAME}: {model_init_e}")
            ...
            if summarized_history_result is not None:
                ...
                final_token_count = await count_history_tokens(final_history, main_model_instance)
                utils.add_debug_log(f"Token count after summarization: {final_token_count}")
            else:
                utils.add_debug_log("Summarization failed. Proceeding with original (long) history.")
        ...
        utils.add_debug_log(f"Error during history token management logic: {hist_mgmt_e}")
        # Return original history and its count on error
        final_token_count = await count_history_tokens(gemini_history, main_model_instance) # Recount just in case
