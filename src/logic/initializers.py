...
def initialize_gemini_client() -> Optional[genai.Client]:
    ...
    if not config.config_valid:
        utils.add_debug_log("Skipping Gemini client initialization due to invalid configuration.")
        return None
    ...
    if not api_key:
        utils.add_debug_log("ERROR: Gemini API Key not available in config during initialization.")
        return None
...
def initialize_mcp_servers() -> Dict[str, MCPServer]:
    ...
    # Check if config validation passed
    if not config.config_valid:
        utils.add_debug_log("Skipping MCP server initialization due to invalid configuration.")
        return mcp_servers
    ...
    if fs_target_dirs: # Only initialize if the list is not empty and paths are valid (checked in config.py)
        try:
            ...
            fs_args.extend(fs_target_dirs) # Add all configured directories to the args list
            ...
            utils.add_debug_log(f"Initialized 'filesystem' MCPServer instance for directories: {fs_target_dirs}")
        except Exception as e:
        ...
        utils.add_debug_log("Skipping 'filesystem' MCPServer initialization: No valid target directories configured.")
    ...
    if config.ENABLE_MEMORY_SERVER: # Check the flag from config
        try:
            ...
            utils.add_debug_log(f"ERROR: Failed to initialize 'memory' MCPServer: {e}\n{traceback.format_exc()}")
    else:
        utils.add_debug_log("Skipping 'memory' MCPServer initialization: 'enable_memory_server' is false in config.")
    # --- END NEW ---
    ...
    if mcp_servers:
        print(f"Initialized {len(mcp_servers)} MCP server configurations: {list(mcp_servers.keys())}")
    else:
